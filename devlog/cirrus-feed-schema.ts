...
export interface ChannelStatics {
  fetchAndUpsert(
    url: string,
  ): Promise<{ channel: Channel; entries: ChannelEntry[] }>;
}

export type ChannelCollection = RxCollection<
  Channel,
  ChannelMethods,
  ChannelStatics
>;
...
export function getChannelEntryCollection(
  database: RxDatabase,
): ChannelEntryCollection {
...
export const channelsRegistration = (
...
): RxCollectionRegistration<Channel> => ({
  ...
  statics: {
    fetchAndUpsert: async function (
      ...
      const upstream = await upstreamChannelSchema.parseAsync(
        await resp.json(),
      );
        ...
        await Promise.all(
          upstream.entries.map(async (entry) => ({
            ...
            fetchedAt: entry.fetchedAt
              ? new Date(entry.fetchedAt).valueOf()
              : undefined,
            publishedAt: entry.publishedAt
              ? new Date(entry.publishedAt).valueOf()
              : undefined,
          })),
        ),
      );
