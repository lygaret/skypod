...
export async function importKeypair(keypair?: JsonWebKeyPair) {
  ...
  const publicKey = await crypto.subtle.importKey('jwk', keypair.publicKey, IdentityKeyAlgo, true, ["verify"])
  const privateKey = await crypto.subtle.importKey('jwk', keypair.privateKey, IdentityKeyAlgo, true, ["sign"])
